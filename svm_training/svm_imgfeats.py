...
print(dset, vtype, vmodel)

feat_dict  = json.load(open('features/image/%s_%s_%s.json'%(dset, vtype, vmodel), 'r'))

test_idxs = np.array([idx for idx in te_df[0]])

ft_train = np.array([feat_dict[vfeat][str(idx)] for idx in tr_df[0]])
ft_val = np.array([feat_dict[vfeat][str(idx)] for idx in vl_df[0]])
ft_test = np.array([feat_dict[vfeat][str(idx)] for idx in te_df[0]])
...
print("SVM %s, Split-%d,  %s-%s-%s"%(dset, split, vtype, vmodel, vfeat))
